...
# Platform detection: real MicroPython sets sys.implementation.name to
# 'micropython'. CPython sets it to 'cpython'. This is a property of the
# Python runtime itself, not an importable module, so the shim cannot
# fake it — unlike attributes on the `micropython` stub module, which
# the shim defines as no-ops for compatibility.
#
# We use this to decide whether to install the viper-accelerated kernels
# (correct on device) or the plain-Python kernels (correct on CPython
# under the shim).
#
